...
export default function PlayScreen() {
  ...
  const flushPlaybackRecord = useCallback(() => {
    const { _savePlayRecord, status } = usePlayerStore.getState();
  ...
  useEffect(() => {
    ...
    return () => {
      logger.info(`[PERF] PlayScreen unmounting - flushing playback record and resetting state`);
      flushPlaybackRecord();
      reset(); // Reset state when component unmounts
    };
  }, [episodeIndex, source, position, reset, loadVideo, id, title, flushPlaybackRecord]);
  ...
  }, [showControls, setShowControls, router, flushPlaybackRecord]);
