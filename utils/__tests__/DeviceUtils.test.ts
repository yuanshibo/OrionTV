...
// Mock react-native's Platform and Dimensions.
// Using a complete, isolated mock is safer than jest.requireActual,
// as it avoids pulling in native dependencies that can crash the test environment.
jest.mock("react-native", () => ({
  Dimensions: {
    get: jest.fn(),
  },
  Platform: {
    isTV: false, // Default mock value that can be changed in tests
  },
}));

const mockedDimensions = Dimensions as jest.Mocked<typeof Dimensions>;
// We need to be able to change Platform.isTV for different tests.
const mockedPlatform = Platform as { isTV: boolean };
...
describe("DeviceUtils", () => {
  ...
  describe("getDeviceType", () => {
    it("应该在 Platform.isTV 为 true 时返回 tv", () => {
      mockedPlatform.isTV = true;
      // Dimensions shouldn't matter if Platform.isTV is the source of truth
    ...
    it("应该在宽度 >= 1024 且 Platform.isTV 为 false 时返回 tv (for web/other platforms)", () => {
  ...
  describe("isTV", () => {
    ...
    it("应该在非 TV 设备上返回 false", () => {
      mockedPlatform.isTV = false;
      // also check with large screen that is not a TV
  ...
  describe("isMobile", () => {
    ...
    it("应该在非移动设备上返回 false", () => {
      mockDimensions(1024, 768); // tablet
  ...
  describe("getOptimalFontSize", () => {
    ...
    it("应该为 tablet 设备返回基础大小 * 1.1", () => {
      mockDimensions(768, 1024);
      expect(Math.round(DeviceUtils.getOptimalFontSize(16))).toBe(18);
    });
  ...
  describe("getOptimalSpacing", () => {
    it("应该为 mobile 设备返回基础间距 * 0.8", () => {
      mockDimensions(375, 812);
      expect(Math.round(DeviceUtils.getOptimalSpacing(20))).toBe(16);
    });
